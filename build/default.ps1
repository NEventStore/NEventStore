...
task PackageAzureTablesPersistence -depends Clean, Compile, PackageEventStore {
	...
	Merge-Assemblies -outputFile "$output_directory/plugins/persistence/azure-tables/EventStore.Persistence.AzureTablesPersistence.dll" -exclude "EventStore.*" -keyfile $keyFile -files @(
