...
public class when_a_commit_header_has_a_name_that_contains_a_period : PersistenceEngineConcern
{
    ...
    protected override void Context()
    {
        ...
        var attempt = new CommitAttempt(_streamId,
            ...
            new[] { new EventMessage { Body = new ExtensionMethods.SomeDomainEvent { SomeProperty = "Test" } } });
...
public class when_reading_from_a_given_revision : PersistenceEngineConcern
{
    ...
    protected override void Because()
    {
        _committed = Persistence.GetFrom(_streamId, LoadFromCommitContainingRevision, UpToCommitWithContainingRevision)
            .ToArray();
    }
...
public class when_adding_multiple_snapshots_for_same_bucketId_streamId_streamRevision : PersistenceEngineConcern
{
    ...
    public void should_be_able_to_retrieve_the_correct_snapshot_original_or_updated_depends_on_driver_implementation()
    {
        ...
        if (_added)
            snapshot.Payload.Should().Be(_updatedSnapshot.Payload,
                "The snapshot was added, I expected to get the most updated version");
        else
            snapshot.Payload.Should().Be(_snapshot.Payload,
                "The snapshot was not added, I excpeted to get the original version");
    }
}
...
public class when_adding_a_commit_after_a_snapshot : PersistenceEngineConcern
{
    ...
    {
        Persistence.GetStreamsToSnapshot(WithinThreshold).FirstOrDefault(x => x.StreamId == _streamId).Should()
            .NotBeNull();
    }
...
public class when_paging_over_all_commits_from_a_particular_checkpoint : PersistenceEngineConcern
{
    ...
    {
        _committed.Skip(checkPoint).All(x => _loaded.Contains(x)).Should()
            .BeTrue(); // all commits should be found in loaded collection
    }
}
...
public class when_paging_over_all_commits_of_a_bucket_from_a_particular_checkpoint : PersistenceEngineConcern
{
    ...
    protected override void Context()
    {
        _committedOnBucket1 = Persistence.CommitMany(ConfiguredPageSizeForTesting + 1, null, "b1")
            .Select(c => c.CommitId).ToList();
        _committedOnBucket2 = Persistence.CommitMany(ConfiguredPageSizeForTesting + 1, null, "b2")
            .Select(c => c.CommitId).ToList();
    ...
    {
        _committedOnBucket1.Skip(checkPoint).All(x => _loaded.Contains(x)).Should()
            .BeTrue(); // all commits should be found in loaded collection
    }
...
public class when_paging_over_all_commits_from_a_particular_checkpoint_to_a_checkpoint : PersistenceEngineConcern
{
    ...
    private readonly List<Guid> _committed = new();
    private ICollection<Guid> _loaded;
    private int endCheckpoint;

    protected override void Context()
    {
        var committedOnBucket1 = Persistence.CommitMany(ConfiguredPageSizeForTesting + 1, null, Bucket.Default)
            .Select(c => c.CommitId).ToList();
        var committedOnBucket2 = Persistence.CommitMany(ConfiguredPageSizeForTesting + 1, null, "Bucket1")
            .Select(c => c.CommitId).ToList();
    ...
    [Fact]
    public void
        should_load_the_same_number_of_commits_which_have_been_persisted_starting_from_the_checkpoint_to_the_checkpoint()
    {
...
#endif
public class
    when_paging_over_all_commits_of_a_bucket_from_a_particular_checkpoint_to_a_checkpoint : PersistenceEngineConcern
{
    private const int startCheckpoint = 2;
    private List<Guid> _committedOnBucket1;
    private List<Guid> _committedOnBucket2;
...
public class when_committing_a_stream_with_the_same_id_as_a_stream_in_another_bucket : PersistenceEngineConcern
{
    ...
    {
        var stream = Persistence.GetFrom(_bucketBId, _streamId, 0, int.MaxValue).ToArray();
    ...
    {
        var stream = Persistence.GetFrom(_bucketAId, _streamId, 0, int.MaxValue).ToArray();
        stream.Should().NotBeNull();
        stream.Count().Should().Be(1);
...
#endif
public class
    when_saving_a_snapshot_for_a_stream_with_the_same_id_as_a_stream_in_another_bucket : PersistenceEngineConcern
{
    private const string _bucketAId = "a";
    private const string _bucketBId = "b";
...
#endif
public class
    when_reading_all_commits_from_a_particular_point_in_time_and_there_are_streams_in_multiple_buckets :
...
#endif
public class
    when_getting_all_commits_since_checkpoint_and_there_are_streams_in_multiple_buckets : PersistenceEngineConcern
{
...
public class when_a_payload_is_large : PersistenceEngineConcern
{
    ...
    public void can_commit()
    {
        ...
        var attempt = new CommitAttempt(
            ...
            new[] { new EventMessage { Body = new string('a', bodyLength) } });
        Persistence.Commit(attempt);

        var commits = Persistence.GetFrom(0).Single();
...
#endif
{
    ...
    protected IPersistStreams Persistence => Fixture.Persistence;
...
public partial class PersistenceEngineFixture : IDisposable
{
    ...
    public void Initialize(int pageSize)
    {
            ...
            {
                Persistence =
 new NEventStore.Diagnostics.PerformanceCounterPersistenceEngine(_createPersistence(pageSize), _trackingInstanceName);
            }
            else
            {
