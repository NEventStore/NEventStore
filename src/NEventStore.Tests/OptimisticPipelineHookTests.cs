...
public class OptimisticPipelineHookTests
{
...
#endif
    public class
        when_committing_with_a_sequence_less_or_equal_to_the_most_recent_sequence_for_the_stream : using_commit_hooks
    {
...
#endif
    public class
        when_committing_with_a_revision_less_or_equal_to_than_the_most_recent_revision_read_for_the_stream :
        using_commit_hooks
    {
...
#endif
    public class
        when_committing_with_a_commit_sequence_less_than_or_equal_to_the_most_recent_commit_for_the_stream :
        using_commit_hooks
    {
...
#endif
    public class
        when_committing_with_a_stream_revision_less_than_or_equal_to_the_most_recent_commit_for_the_stream :
        using_commit_hooks
    {
    ...
    public class when_tracking_commits : SpecificationBase
    {
        ...
        protected override void Because()
        {
            foreach (var commit in _trackedCommitAttempts) _hook.Track(commit);
        }

        [Fact]
        public void should_only_contain_streams_explicitly_tracked()
        {
            var untracked = BuildCommit(4, Guid.Empty, _trackedCommitAttempts[0].CommitId);
        ...
        public void should_find_tracked_streams()
        {
            ...
            var stillTracked = BuildCommit(lastCommit.CheckpointToken, lastCommit.StreamId, lastCommit.CommitId);
        ...
        public void should_only_track_the_specified_number_of_streams()
        {
            ...
            var droppedFromTracking =
                BuildCommit(firstCommit.CheckpointToken, firstCommit.StreamId, firstCommit.CommitId);
    ...
    {
        protected readonly OptimisticPipelineHook Hook = new();
        ...
        protected ICommit BuildCommitStub(long checkpointToken, int streamRevision, int commitSequence)
        {
            ...
            return new Commit(Bucket.Default, _streamId, streamRevision, Guid.NewGuid(), commitSequence,
                SystemTime.UtcNow, checkpointToken, null, events);
        }

        protected CommitAttempt BuildCommitAttemptStub(int streamRevision, int commitSequence)
        {
            var events = new[] { new EventMessage() };
            return new CommitAttempt(Bucket.Default, _streamId, streamRevision, Guid.NewGuid(), commitSequence,
        ...
        {
            var events = new[] { new EventMessage() }.ToList();
            return new Commit(Bucket.Default, _streamId, streamRevision, commitId, commitSequence, SystemTime.UtcNow,
