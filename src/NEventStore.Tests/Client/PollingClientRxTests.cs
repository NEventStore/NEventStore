namespace NEventStore.Client
{
    ...
    public class CreatingPollingClientTests
    {
        ...
        {
            Catch.Exception(() => new PollingClientRx(null)).ShouldBeInstanceOf<ArgumentNullException>();
        }

        [Fact]
        public void When_interval_less_than_zero_then_should_throw()
        {
            Catch.Exception(() => new PollingClientRx(A.Fake<IPersistStreams>(), -1)).ShouldBeInstanceOf<ArgumentException>();
        }

        [Fact]
        public void When_interval_is_zero_then_should_throw()
        {
            Catch.Exception(() => new PollingClientRx(A.Fake<IPersistStreams>(), 0)).ShouldBeInstanceOf<ArgumentException>();
        }
    }
