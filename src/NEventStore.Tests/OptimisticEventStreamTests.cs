...
public class when_committing_any_uncommitted_changes : on_the_event_stream
{
    ...
    protected override void Context()
    {
        ...
        foreach (var item in _headers) Stream.UncommittedHeaders[item.Key] = item.Value;
    }
...
/// <summary>
///     This behavior is primarily to support a NoSQL storage solution where CommitId is not being used as the "primary
///     key"
...
public class when_committing_after_another_thread_or_process_has_moved_the_stream_head : on_the_event_stream
{
    ...
    protected override void Context()
    {
        ...
        A.CallTo(() => Persistence.GetFrom(BucketId, StreamId, StreamRevision + 1, int.MaxValue))
...
#endif
{
    ...
    protected ICommitEvents Persistence => _persistence ?? (_persistence = A.Fake<ICommitEvents>());

    protected OptimisticEventStream Stream
    {
        get => _stream ?? (_stream = new OptimisticEventStream(BucketId, StreamId, Persistence));
        set => _stream = value;
    }
    ...
    protected ICommit BuildCommitStub(long checkpointToken, int revision, int sequence, int eventCount)
    {
        ...
        return new Commit(Bucket.Default, StreamId, revision, Guid.NewGuid(), sequence, SystemTime.UtcNow,
