namespace NEventStore
{
    ...
    internal static class Guard
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException(GetParameterName(reference));
        }

        internal static void NotNull<T>(Expression<Func<T>> reference, T value)
        {
            if (value == null) throw new ArgumentNullException(GetParameterName(reference));
        }
            ...
            where T : IComparable
        {
            ...
            if (value.CompareTo(compareTo) <= 0)
                throw new ArgumentOutOfRangeException(
                    "{0} has value {1} which is less than or equal to {2}".FormatWith(GetParameterName(reference),
                        value, compareTo));
        }
            ...
            where T : IComparable
        {
            ...
            if (value.CompareTo(compareTo) < 0)
                throw new ArgumentOutOfRangeException(
                    "{0} has value {1} which is less than {2}".FormatWith(GetParameterName(reference), value,
                        compareTo));
        }
